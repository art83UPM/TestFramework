...
public class DataReaderWriter implements Visitor {
	...
	public void visit(ProjectClazz clazz) {
		this.file = new File(path + clazz.getPackagePath() + File.separator
				+ "_dataReaders" + File.separator + clazz.getName()
				+ "Test.java");
		try {
			...
			writer.write("import " + clazz.getProjectPackage().getName() + " "
					+ clazz.getName() + ";\n\n");
			writer.write("public class " + clazz.getName()
					+ "TestDataReader extends TestDataReader {\n\n");
			writer.write(this.printTabs(1) + "private " + clazz.getName() + " "
					+ clazz.getName().toLowerCase() + ";\n\n");
			writer.write(this.printTabs(1)
					+ "private final static String[] CONSTRUCTOR_NAMES = {");
			int i = 0;
			for (ProjectConstructorMember constructor : clazz
					.getConstructorMemberList()) {
			...
			writer.write(this.printTabs(1) + "public " + clazz.getName()
					+ "TestDataReader() {\n");
			writer.write(this.printTabs(2) + "super(" + this.resourcesPath
					+ clazz.getPackagePath() + clazz.getName()
					+ "TestData.xlsx" + ");\n");
			writer.write(this.printTabs(1) + "}\n\n");
			...
			for (ProjectConstructorMember constructor : clazz
					.getConstructorMemberList()) {
				writer.write(this.printTabs(2) + "case \""
						+ constructor.getNameWithParams() + "\":\n");
				writer.write(this.printTabs(3) + "return existsConstructor"
						+ constructor.getNameWithParams() + "();\n\n");
			}
			...
			for (ProjectConstructorMember constructor : clazz
				...
				writer.write(this.printTabs(2) + "case \""
						+ constructor.getNameWithParams() + "\":\n");
				writer.write(this.printTabs(3) + "construct"
						+ constructor.getNameWithParams() + "();\n");
			...
			writer.write(this.printTabs(1) + "public " + clazz.getName()
					+ " get" + clazz.getName() + "() {\n");
			writer.write(this.printTabs(2) + "return this."
					+ clazz.getName().toLowerCase() + ";\n");
			writer.write(this.printTabs(1) + "}\n\n");
	...
	public void visit(ProjectConstructorMember constructorMember) {
		try {
			writer.write(this.printTabs(1)
					+ "private boolean existsConstructor"
					+ constructorMember.getNameWithParams() + "() {\n");
			writer.write(this.printTabs(2) + "try {\n");
			if (constructorMember.getParameterNumber() == 0) {
				writer.write(this.printTabs(3)
						+ "String x = this.getString(\"get"
						+ constructorMember.getNameWithParams() + "\");\n");
				writer.write(this.printTabs(3)
						+ "if (!x.equalsIgnoreCase(\"x\")) {\n");
				writer.write(this.printTabs(4)
						+ "throw new InvalidDataReaderException(\"Data under column \\\"get"
						+ constructorMember.getNameWithParams()
				...
				for (int i = 0; i < constructorMember.getParameterNumber(); i++) {
					writer.write(this.printTabs(3) + "this.getInt(\"get"
							+ constructorMember.getNameWithParams()
							+ "Parameter" + i + "\");\n");
				}
			}
			...
			writer.write(this.printTabs(1) + "private void construct"
					+ constructorMember.getNameWithParams() + "() {\n");
			if (constructorMember.getParameterNumber() == 0) {
				writer.write(this.printTabs(2) + "this."
						+ Capitalizer.unCapitalize(constructorMember.getName())
						+ "= new " + constructorMember.getName() + "();\n");
			} else {
				writer.write(this.printTabs(2) + "try {\n");
				for (int j = 0; j < constructorMember.getParametersType()
						.size(); j++) {
					ProjectParameterMember parameter = constructorMember
							.getParametersType().get(j);
					writer.write(this.printTabs(3) + parameter.getType()
							+ constructorMember.getNameWithParams()
							+ "Parameter" + j + " = this.get"
							+ Capitalizer.capitalize(parameter.getType())
							+ "(\"get" + constructorMember.getNameWithParams()
							+ "Parameter" + j + "\");\n");
				}
				writer.write(this.printTabs(2) + "this."
						+ Capitalizer.unCapitalize(constructorMember.getName())
						+ "= new " + constructorMember.getName() + "(");
				for (int k = 0; k < constructorMember.getParametersType()
						.size(); k++) {
					writer.write(constructorMember.getNameWithParams()
							+ "Parameter" + k);
